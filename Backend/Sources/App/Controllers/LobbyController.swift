...
class LobbyController: NetworkDelegate {
    ...
    private func onStartRequest(by user: User) async {
        guard joinedUsers.contains(where: { u in u == user }) else {
            await networkManager.send(
                msg: ProtoError(code: .userNotYetJoined, message: "You cannot start the game since you didn't join the lobby yet."),
                to: user
            )
            return
        }
        ...
        await synced.run {
            ...
            guard !self.gameRunning else {
                await networkManager.send(
                    msg: ProtoError(code: .gameAlreadyStarted, message: "You cannot start the game since it is already running."),
                    to: user
                )
                return
            }
            ...
            guard self.joinedUsers.count >= Constants.JOINED_USER_START_NR else {
                await networkManager.send(
                    msg: ProtoError(code: .notEnoughPlayers, message: "You cannot start the game since there are not enough players in the lobby."),
                    to: user
                )
                return
            }
    ...
    private func broadcastLobbyUpdate() async {
        ...
        let lobbyUpdate = ProtoUpdate(data:
